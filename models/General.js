...
const General = {
    ...
    getUniqueKey: async function () {
        ...
        let id          = 'GENERAL' + machine.machineIdSync() + time + random;
        let resultExist = query("SELECT `id` FROM `general` WHERE `id` = '" + id + "'");
        let cl          = this;
    ...
    /**
     * Get Generals list against the given dates
     * @param start
     * @param end
     * @returns {Promise.<*>}
     */
    getGenerals: async function (start, end) {
        start = moment(start, 'DD/MM/YYYY').format('YYYY-MM-DD');
        end   = moment(end, 'DD/MM/YYYY').format('YYYY-MM-DD');
        return await query("SELECT * FROM `general` WHERE `deleted_at` IS NULL " +
     ...
     * @returns {Promise.<*>}
     */
    createGeneral: async function (data, created) {
        return await this.getUniqueKey().then(function (id) {
            ...
            return query("INSERT INTO `general`(`id`, `account`, `debit`, `credit`, `description`, `created_by`, `created_at`, `updated_at`) " +
     ...
     * @returns {Promise.<*>}
     */
    updateGeneral: async function (data) {
        const updated_at = moment().format('YYYY-MM-DD HH:mm:ss');
        return await query("UPDATE `general` SET " +
