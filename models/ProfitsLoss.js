...
const ProfitsLoss = {
    ...
    getUniqueKey: async function () {
        ...
        let id          = 'PROFIT' + machine.machineIdSync() + time + random;
        let resultExist = query("SELECT `id` FROM `profits` WHERE `id` = '" + id + "'");
        let cl          = this;
    ...
    /**
     * Get Profits and Loss list against the given dates
     * @param start
     * @param end
     * @returns {Promise.<*>}
     */
    getProfitsLosses: async function (start, end) {
        start = moment(start, 'DD/MM/YYYY').format('YYYY-MM-DD');
        end   = moment(end, 'DD/MM/YYYY').format('YYYY-MM-DD');
        return await query("SELECT * FROM `profits` WHERE `deleted_at` IS NULL " +
                           "AND DATE(`created_at`) >= '" + start + "' AND DATE(`created_at`) <= '" + end + "' " +
                           "ORDER BY `created_at`");
    },

    /**
     * Create Profits and Loss
     * @param data
     * @param created
     * @returns {Promise.<*>}
     */
    createProfitsLoss: async function (data, created) {
        return await this.getUniqueKey().then(function (id) {
            ...
            return query("INSERT INTO `profits`(`id`, `account`, `debit`, `credit`, `description`, `created_by`, `created_at`, `updated_at`) " +
                         "VALUES('" + id + "', '" + data['account'] + "', '" + data['debit'] + "', '" + data['credit'] + "', " +
     ...
     * @returns {Promise.<*>}
     */
    updateProfitsLoss: async function (data) {
        const updated_at = moment().format('YYYY-MM-DD HH:mm:ss');
        return await query("UPDATE `profits` SET " +
