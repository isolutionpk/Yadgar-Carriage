...
const token   = '0416316b9c59b2e7cddb4669328deccf6d90af76'
...
function zipAssets() {
    ...
    const zipAssets = [{
        name: 'yadgar-carriage-windows.zip',
        path: path.join(outPath, 'YadgarCarriage')
    }]

    return Promise.all(zipAssets.map(zipAsset)).then((zipAssets) => {
        return zipAssets.concat([{
            name: 'RELEASES',
            path: path.join(outPath, 'YadgarCarriage', 'RELEASES')
        }, {
            name: 'YadgarCarriageSetup.exe',
            path: path.join(outPath, 'YadgarCarriage', 'YadgarCarriageSetup.exe')
        }, {
            name: `YadgarCarriage-${version}-full.nupkg`,
            path: path.join(outPath, 'YadgarCarriage', `YadgarCarriage-${version}-full.nupkg`)
        }])
    })
}
...
function createRelease(assets) {
    const options = {
        uri: 'https://api.github.com/repos/isolutionpk/Yadgar-Carriage/releases',
        headers: {
